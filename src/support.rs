...
pub struct FrameData<'f, 'ui> {
    pub ui: &'f mut Ui<'ui>,
    pub encoder: &'f mut types::Encoder,
    pub render_sys: &'f mut RenderSystem,
}
...
impl System {
    pub fn main_loop<F: FnMut(&mut bool, FrameData)>(self, mut run_ui: F) {
        let System {
            mut events_loop,
            mut imgui,
            mut platform,
        ...
        while run {
            ...
            run_ui(&mut run, FrameData { encoder: &mut encoder, ui: &mut ui, render_sys: &mut render_sys });
